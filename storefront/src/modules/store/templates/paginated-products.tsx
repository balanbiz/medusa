...
import ProductRandomBtn from "@modules/products/components/product-random-btn"
...
export default async function PaginatedProducts({
  ...
  async function getFullProducts() {
    try {
      const fullProductPromises = products.map(async (fullProduct) => {
        const result = await getProductByHandle(fullProduct.handle!)
        return result
      })
    ...
    <>
      ...
      >
        ...
        <li className="flex justify-center items-center">
          <ProductRandomBtn fullProducts={fullProducts} />
        </li>
      </ul>
      {totalPages > 1 && (
        <Pagination
